...
状态：Target Domain、Sizing、共享 Output 与 `ImagePipeline` 反馈搜索已实施。
...
Resolver、Solver 和 ExecutionPlan 的旧描述均已被替代，这些架构类型已删除。
...
Quality 不属于 Output；它由 Target 搜索算法所有，并且不能在搜索期间改变 representation、
...
因此搜索算法不再搜索彼此独立的 width 和 height，也不再区分 1.x hard/soft
geometry。它也不参与裁切决策：调用方没有声明 aspect ratio 时不裁切；声明后，crop
rect 在搜索前固定。搜索算法只能缩放该裁切结果，不能为了更容易满足 `maxBytes` 改变
ratio、anchor、crop rect 或额外丢弃画面内容。
...
Target 保留结构化结果，因为实际输出由反馈搜索决定。结果至少需要让调用方取得：
...
- Quality 不属于 Output，由 Target 搜索算法所有。
- Sizing 解析后，搜索算法只调整统一缩放系数与 quality，不改变固定 crop。
- Public API 不暴露 candidate preference 或搜索参数。
...
3. 实现 Target 反馈搜索
   scale + quality candidates -> ImagePipeline encode -> measure/select
