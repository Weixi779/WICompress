...
Real image fixtures for `CharacterizationTests`. **Drop images here and they are
...
that must survive the planned UIKit → ImageIO core rewrite (see
`docs/PLAN_v1.0.0.md`).
...
| `real_heic_2995x3788_alpha.heic` | **透明(alpha)** | 2 → 1497×1894 | HEIC 透明保留;未来 alpha 处理基线 |
...
- **所有真实图都是 Display P3**(iPhone 来源)→ 正好用来锁「色彩 profile 保留」。
- **HDR gain map**:四张相机 HEIC 里有三张带 gain map → 未来 gain-map preserve 特性的基线。
- `recompressed` 那张是**故意保留**的「已压缩小图」:它是唯一能触发「重编码反而变大」的样本,别替换。详见 `docs/PLAN_v1.0.0.md` §13。
- **尚缺**:多帧/动图样本(用于 `animatedSourceUnsupported` 抛错测试)。建议日后放进 `Resources/Unsupported/` 子目录,并让自动发现的契约测试跳过该目录。
